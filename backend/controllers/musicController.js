...
// Calm Space page for a quick "I feel X right now" without a full journal entry.
export function suggestMusic(req, res, next) {
  try {
    const { mood, note, genre } = req.body;
    ...
    const suggestion = buildSuggestion({ mood, intensity: 3, note, genre });
